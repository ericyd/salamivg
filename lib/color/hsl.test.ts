...
describe('ColorHsl', () => {
  describe('fromRgb', () => {
    ...
    const inexactTests = [
      ['#555a99', { h: 236, s: 0.29, l: 0.47 }],
      ['#4F1349', { h: 306, s: 0.61, l: 0.19 }],
    ]
  ...
  describe('mix', () => {
    ...
    // if there wasn't a correction for range [0, 360], the hue would be >360
    it('handles positive overflow', () => {
      const a = new ColorHsl(359, 1, 1)
      const b = new ColorHsl(20, 1, 1)
      const acutal = a.mix(b, 0.4)
      assert.strictEqual(Math.round(acutal.h), 7)
    })

    // if there wasn't a correction for range [0, 360], the hue would be <0
    it('handles negative overflow', () => {
      const a = ColorRgb.fromHex('#874059ff').toHsl()
      const b = ColorRgb.fromHex('#55598aff').toHsl()
      const actual = a.mix(b, 0.2)
      assert.strictEqual(Math.round(actual.h), 318)
    })
  })
})
