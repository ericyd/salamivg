...
describe('smallestAngularDifference', () => {
  const tests = [
    ...
    { angle1: -Math.PI * 9 / 2, angle2: Math.PI * 3 / 2, expected: 0},
    { angle1: Math.PI * 3/2, angle2: -Math.PI / 2, expected: 0},
    { angle1: Math.PI * 3/2, angle2: -Math.PI * 9 / 2, expected: 0},
  ...
  for (const { angle1, angle2, expected } of approximateTests) {
    it(`should return ${expected} for ${angle1} and ${angle2}`, () => {
      ...
      assert(isWithin(expected - 0.001,  expected + 0.001,  actual), `Actual: ${actual}, Expected: ${expected}`)
    })
  }
})
